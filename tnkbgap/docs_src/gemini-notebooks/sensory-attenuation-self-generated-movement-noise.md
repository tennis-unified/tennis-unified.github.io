# Sensory Attenuation & Self-Generated Noise: Motor Cortex Corollary Discharge Suppressing Distraction

**Author:** Henry Phạm Đức · Tennis Future Lab & Kinetic Biomechanics Research  
**Domain:** Neurological Control, Visual Processing & Reaction Time  
**Source Vaults:** `The Hidden Engine of Elite Tennis Performance · Neuro-Ocular Studies`  
**Keywords:** `Sensory Attenuation, Corollary Discharge, Efference Copy, Self-Generated Movement Noise, Auditory Gating`  

---

## Executive Abstract

Every movement a tennis player makes creates massive internal sensory noise: the sound of squeaking shoes, the visual motion of their own racket, and proprioceptive vibrations. Through **Sensory Attenuation and Corollary Discharge**, the motor cortex sends an inhibitory copy of the motor command to the sensory cortices, mathematically subtracting self-generated noise to amplify subtle external cues (the sound of the opponent's ball impact).

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    NEURO-OCULAR & SENSORIMOTOR PROCESSING ARCHITECTURE      │
│                                                                             │
│ [Phase 1: Retinal Ganglion M-Cell Transduction (< 30ms)]                    │
│                                  │                                          │
│ [Phase 2: Dorsal Stream Optic Flow & VOR Stabilization]                     │
│                                  │                                          │
│ [Phase 3: Cerebellar Forward Model Predictive Simulation (< 120ms)]          │
│                                  │                                          │
│ [Phase 4: Subcortical Motor Engram Discharge] ──► ⚡ [Pre-Impact Strike]     │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 1. Corollary Discharge Inhibitory Gating

The motor cortex tells the auditory and somatosensory cortices: 'Ignore my own shoe squeak and racket whoosh.' This increases signal-to-noise ratio by 18 dB.

```
       [ Visual Sensory Input ] ──► [ Magnocellular High-Speed Pathway ]
                                                  │
       [ Motor Execution Engram ] ◄───────────────┘
        (Bypassing 150ms Cortical Latency)
```

---

## 2. The Acoustic Signature of Ball Spin

Suppression of internal noise allows elite players to hear the high-frequency acoustic friction of topspin vs. the flat pop of a flat drive in under 20ms.

---

## 3. Auditory Gating Sensitivity Drills

Blindfolded sound-localization ball strikes; acoustic pitch differentiation of incoming spin rates.

---

## Neurological Diagnostic & Remediation Matrix

| Neuro-Visual Metric | Common Perceptual Fault | Clinical / Tactical Risk | Prescribed Intervention Protocol |
|---|---|---|---|
| **Gaze Stability** | Erratic saccades chasing ball | Severe frame shanking & motion blur | **Quiet Eye Horizon Anchor**: Lock gaze on contact zone for 150ms. |
| **Reaction Latency** | Waiting for post-bounce visual cues | Consistently late on 120+ mph serves | **Perceptual Occlusion Drills**: Decode server toss & shoulder tilt. |
| **Mental Interference** | Left-hemisphere verbal self-talk | Motor choking & stroke deceleration | **Alpha-Theta Somatic Grounding**: Focus on breath & foot pressure. |
